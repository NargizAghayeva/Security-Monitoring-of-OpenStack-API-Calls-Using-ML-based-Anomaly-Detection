...
5. [Phase 1: Keystone API Anomaly Detection](#Phase-1-keystone-api-anomaly-detection)
6. [Phase 2: Nova Log Anomaly Detection](#Phase-2-nova-log-anomaly-detection)
7. [Results](#results)
...
Both Phases use an autoencoder trained exclusively on normal data. Anomalies are detected by measuring reconstruction error: normal inputs reconstruct with low error, anomalous inputs with high error. A threshold derived from the normal error distribution separates the two classes.

### Key Results

| Phase | Dataset | F1 Score | ROC-AUC |
|-------|---------|----------|---------|
| Phase 1 — Keystone | CICIDS2017 + real OpenStack Keystone logs | 0.9981 | 1.0000 |
| Phase 2 — Nova | ParisaKalaki OpenStack Nova logs | 0.9900 | 0.9995 |
...
│
├── PHASE 1: CICIDS2017/                 # Phase 1 — CICIDS2017 pretraining + Keystone fine-tuning
...
│
└── PHASE 2: Nova anomaly detection/     # Phase 2 — Nova log anomaly detection
    └── Nova_anomaly_detection.md
```
...
### OpenStack (Phase 1 dataset generation only)

Phase 1 dataset generation requires a running OpenStack environment. A single-node DevStack setup on Ubuntu 22.04 was used. Full setup notes and troubleshooting are in `01-devstack-setup/`.

## Dataset Generation

### Phase 1 — Keystone API Dataset
...
This prints row counts, label distribution, and attack type breakdown. The output file `final_dataset.csv` is the input for Phase 1 Phase 2.

### Phase 2 — Nova Log Dataset
...
## Phase 1: Keystone API Anomaly Detection

Phase 1 uses transfer learning. An autoencoder is pretrained on the CICIDS2017 network intrusion dataset, then fine-tuned on real OpenStack Keystone session data collected from a DevStack environment.
...
## Phase 2: Nova Log Anomaly Detection

Phase 2 operates directly on raw OpenStack Nova log files. No pretrained model or external dataset download is required beyond the Nova logs cloned automatically in Cell 2.

Full step-by-step code: `PHASE 2: Nova anomaly detection/phase2_nova_anomaly_detection.md`
...
Both phases demonstrate that unsupervised autoencoder-based anomaly detection is effective for OpenStack API security monitoring without requiring labelled attack data during training.

### Summary Table

| Phase | Method | Features | F1 | ROC-AUC |
|-------|--------|----------|----|---------|
| Phase 1 — Keystone | Transfer learning (CICIDS2017 → Keystone) | 24 session features | 0.9981 | 1.0000 |
| Phase 2 — Nova | Autoencoder on sliding window log features | 14 window features | 0.9900 | 0.9995 |
...
title: Security Monitoring of OpenStack API Calls Using ML-Based Anomaly Detection
summary: Unsupervised autoencoder-based anomaly detection for OpenStack Keystone and Nova APIs. Phase 1 uses transfer learning from CICIDS2017 achieving F1=0.9981 and ROC-AUC=1.0000. Phase 2 detects Nova log anomalies achieving F1=0.9900 and ROC-AUC=0.9995.
